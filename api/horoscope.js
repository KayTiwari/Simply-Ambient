// Vercel serverless proxy for the daily/weekly/monthly horoscope API. Same reason as
...
//   period = daily | weekly | monthly   sign = <zodiac name>   day = TODAY (daily only)
export default async function handler(req, res) {
  res.setHeader('Access-Control-Allow-Origin', '*');
  res.setHeader('Cache-Control', 'no-store');
  try {
    const requestedPeriod = String(req.query.period || 'daily');
    const period = requestedPeriod === 'weekly' || requestedPeriod === 'monthly'
      ? requestedPeriod
      : 'daily';
    ...
    const url =
      period !== 'daily'
        ? `${base}/${period}?sign=${encodeURIComponent(sign)}`
