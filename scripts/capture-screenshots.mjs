...
// Finish the walkthrough: intent -> profile -> recommendations -> tips.
...
await tap('ENTER', { exact: true });      // tips -> app
...
// --- 3. Soundscapes layered under the tone ---------------------------------
...
// --- 4. Settings: privacy card + rate + replay ------------------------------
...
// --- 5. Breath session, mandala visualization -------------------------------
await tap('Breath', { exact: true });     // tab bar
await tap('Box Breathing');
await tap('Mandala');                     // visualization toggle
await tap('PLAY', { exact: true });
